# =====================================================================
# CÓDIGO COMPLETO PARA ENTRENAMIENTO EN COLAB PRO+ (H100)
# Incluye: autenticación, keep-alive, sanity check, pipeline optimizado
# =====================================================================
...
# 2. Keep-Alive (evita que Colab se duerma)
# Ejecuta un clic virtual cada 60 segundos en el botón de conexión
output.eval_js('''
new Promise(resolve => {
  setInterval(() => {
    let btn = document.querySelector("colab-connect-button");
    if (btn) btn.click();
    console.log("Keep-alive: manteniendo conexión activa");
  }, 60000);
  resolve("keep-alive iniciado");
});
''')
print("✅ Keep-Alive activado. Puedes cerrar la pestaña o apagar el PC.\n")

# 3. Imports y configuración del entrenamiento
...
# =====================================================================
# 2. TUBERÍA DE DATOS (CON PARALELISMO AVANZADO Y REPEAT)
# =====================================================================
...
# Tubería de Entrenamiento Optimizada V4.1
dataset_entrenamiento = (archivos_train
           ...
           .shuffle(2000)
           .repeat()  # Suministro infinito para evitar cortes abruptos
           .batch(BATCH_SIZE)
           .prefetch(AUTOTUNE))

# Tubería de Validación Optimizada V4.1
...
# (Alineado con el paralelismo de la H100)
# =====================================================================
...
# 🌟 SOLUCIÓN QUIRÚRGICA: Forzar el cálculo en la CPU para evitar el bug de CUDA
...
def construir_unet(input_shape=(128, 128, 10)):
    ...
    # Retorno obligatorio a float32 para evitar NaN
...
callbacks_list = [
    ...
    callbacks.ModelCheckpoint(filepath=RUTA_MODELO_FINAL, monitor='val_loss', save_best_only=True, verbose=1)
]
...
# Después del fit
total_epocas = len(historial.history['loss'])
if modelo_unet.stop_training:
    print(f"✅ Entrenamiento detenido por Early Stopping en época {total_epocas} (no mejoró en 10 épocas).")
else:
    print(f"✅ Entrenamiento completó todas las épocas: {total_epocas}/80.")
print(f"🏆 Mejor modelo guardado en: {RUTA_MODELO_FINAL}")

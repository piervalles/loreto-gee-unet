...
# =====================================================================
# 1. CONFIGURACIÓN, LOGGING Y CONTROL DE INFERENCIA
# =====================================================================
...
logger = logging.getLogger("Loreto_Inferencia_Pipeline")

@dataclass
class GEEConfigInferencia:
    """Configuración central para la extracción masiva de datos nuevos (Inferencia)."""
    project_id: str = 'proyecto-ia-496311'
    gcs_bucket: str = 'dataset-tfrecords-loreto'
    
    # Nombre del directorio de salida para inferencia 2026 (NUEVAS RUTAS)
    output_folder: str = 'datos_inferencia_2026'
    output_name: str = 'tensor_loreto_1_2026_' 
    
    # Temporalidad controlada (Primer semestre 2026)
...
def build_optical_composite(config: GEEConfigInferencia) -> ee.Image:
    logger.info("Generando compuesto óptico Sentinel-2 (Mediana Semestral 2026)...")
...
def build_radar_composite(config: GEEConfigInferencia) -> ee.Image:
...
def load_dummy_labels(config: GEEConfigInferencia, base_image: ee.Image) -> ee.Image:
    """
    TRUCO MAESTRO: Genera una banda 'LABEL' vacía (ceros) para engañar al 
    analizador de TFRecords en Colab y mantener la estructura del tensor intacta.
    """
    logger.info("Generando banda LABEL dummy (ceros) para compatibilidad del TFRecord...")
    etiqueta_dummy = ee.Image.constant(0).clip(config.roi).rename('LABEL')
    return base_image.addBands(etiqueta_dummy.float())

def export_pipeline_to_gcs(tensor_image: ee.Image, config: GEEConfigInferencia) -> None:
    ruta_destino = f'{config.output_folder}/{config.output_name}'
    ...
    task = ee.batch.Export.image.toCloudStorage(
        ...
        description='Generacion_Inferencia_TFRecords_Loreto_2026',
        bucket=config.gcs_bucket,
        fileNamePrefix=ruta_destino,
        region=config.roi.bounds(), 
        scale=10, 
        fileFormat='TFRecord',
        maxPixels=1e12, 
        formatOptions={
    ...
    task.start()
    logger.info("¡La orden masiva ha sido inyectada en los servidores de Google Earth Engine!")
...
def main():
    logger.info("=== INICIANDO PIPELINE DE EXTRACCIÓN GEE (INFERENCIA 2026) ===")
    
    cfg = GEEConfigInferencia()
    init_gee(cfg.project_id)
    
    # Asignamos el polígono específico de Loreto
    coords_poligono = [
        [
    ...
    # Fusión matemática del Tensor (10 canales limpios)
    features_tensor = optical_features.select(['B2', 'B3', 'B4', 'B8', 'NDVI', 'NDWI', 'NDTI', 'EVI']) \
                                      .addBands(radar_features.select(['VV', 'VH']))
    
    # Acople de etiqueta DUMMY (Ceros) para producción 2026
    final_dataset = load_dummy_labels(cfg, features_tensor)

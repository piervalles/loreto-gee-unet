...
class GEEConfigProduccion:
    ...
    output_name: str = 'tensor_loreto_2_' 
...
def build_optical_composite(config: GEEConfigProduccion) -> ee.Image:
    logger.info("Generando compuesto óptico Sentinel-2 en el área de estudio (ROI)...")
...
def export_pipeline_to_gcs(tensor_image: ee.Image, config: GEEConfigProduccion) -> None:
    # Aislamos los datos de prueba en una carpeta limpia
...
def main():
    ...
    # Asignamos el polígono específico provisto por el usuario
    coords_poligono = [
        [
